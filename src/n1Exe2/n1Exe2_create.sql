...
-- -----------------------------------------------------
-- Table `pizza`.`purchase_order`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `pizza`.`purchase_order` (
  `id` INT NOT NULL,
...
CREATE INDEX `FK_order_client_id_idx` ON `pizza`.`purchase_order` (`order_client_id` ASC) VISIBLE;

CREATE INDEX `FK_store_id_idx` ON `pizza`.`purchase_order` (`store_id` ASC) VISIBLE;
...
CREATE TABLE IF NOT EXISTS `pizza`.`delivery_details` (
  ...
  CONSTRAINT `FK_delivery_order_id`
    ...
    REFERENCES `pizza`.`purchase_order` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION);
