...
![Optic](src/n1_Exe1/s2T1N1Exe1_EER_diagram.png)
...
![Optic](src/n1_Exe2/s2T1N1Exe2_EER_diagram.png)
...
We store a unique identifier for each user: OK
...
A user publishes videos. For each video we save a unique identifier: OK
...
The name of the video file. okay

Video length. okay

A thumbnail. okay

The number of reproductions. okay

The number of likes. okay

The number of dislikes. okay

A video can have three different states: public, hidden and private.
A video can have many tags. okay
A tag is identified by a unique identifier and a tag name. okay
It is interesting to save who the user is who publishes the video and what date/time they do so. okay

A user can create a channel. A channel has a unique identifier: OK
...
A user can subscribe to other users' channels. okay
...
• A user can follow many artists. An artist can be related to other artists who make similar music. So Spotify can show us a list of artists related to the artists we like. We are also interested in saving a user's favorite albums and songs. A user can select many albums and many songs as favorites.
